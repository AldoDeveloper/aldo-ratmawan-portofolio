...
import { CertificationRequest, CertificationResponse } from "../../../types/certification.api";
...
interface UpdateCertificationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (data: CertificationRequest) => Promise<void>;
  data: CertificationResponse | null;
}

export const UpdateCertificationModal: React.FC<UpdateCertificationModalProps> = ({
  isOpen,
  onClose,
  onUpdate,
  data,
  ...
  const onSubmit = async (formData: CertificationRequest) => {
  ...
  return (
    <Modal
      ...
      footer={
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            type="submit"
            form="certification-update-form"
            disabled={isSubmitting}
    ...
    >
      {data && (
        <form id="certification-update-form" onSubmit={handleSubmit(onSubmit)}>
          <CertificationForm
            register={register}
            errors={errors}
            data={data}
