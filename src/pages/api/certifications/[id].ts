import { deleteRequestApi, getRequestApi, patchRequestApi } from "@/utils/request.api.context";
import { NextApiRequest, NextApiResponse } from "next";
import {
    CertificationRequest,
    CertificationDetailResponse,
    CertificationUpdateResponse,
    CertificationDeleteResponse
} from "../../../../types/certification.api";

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const method = req.method;
    const { id } = req.query;

    if (!id || typeof id !== 'string') {
        return res.status(400).json({
            success: false,
            message: 'Certification ID is required'
        });
    }

    try {
        // GET - Fetch single certification by ID
        if (method === 'GET') {
            const result = await getRequestApi<CertificationDetailResponse>(`certifications/${id}`);
            return res.status(200).json(result);
        }

        // PUT/PATCH - Update certification
        if (method === 'PUT' || method === 'PATCH') {
            const body: CertificationRequest = req.body;

            // Validate required fields
            if (!body.name || !body.issuer) {
                return res.status(400).json({
                    success: false,
                    message: 'Name and issuer are required'
                });
            }

            const result = await patchRequestApi<CertificationUpdateResponse, CertificationRequest>(
                `certifications/${id}`,
                undefined,
                body
            );
            return res.status(200).json(result);
        }

        // DELETE - Delete certification
        if (method === 'DELETE') {
            const result = await deleteRequestApi<CertificationDeleteResponse, {}>(
                `certifications/${id}`
            );
            return res.status(200).json(result);
        }

        // Method not allowed
        return res.status(405).json({
            success: false,
            message: 'Method not allowed'
        });

    } catch (err: any) {
        console.error('Certification API Error:', err);
        return res.status(500).json({
            success: false,
            message: err?.message || 'Internal server error',
            error: process.env.NODE_ENV === 'development' ? err : undefined
        });
    }
}
