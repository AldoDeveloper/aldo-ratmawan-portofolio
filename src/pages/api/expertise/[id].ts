...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        // GET - Fetch single expertise item by ID
        if (method === 'GET') {
            const result = await getRequestApi<ExpertiseDetailResponse>(`expertise/${id}`);
            return res.status(200).json(result);
        }

        // PUT/PATCH - Update expertise item
        if (method === 'PUT' || method === 'PATCH') {
            ...
            const result = await patchRequestApi<ExpertiseUpdateResponse, ExpertiseRequest>(
                `expertise/${id}`,
                undefined,
                body
            );
        ...
        if (method === 'DELETE') {
            const result = await deleteRequestApi<ExpertiseDeleteResponse, {}>(
                `expertise/${id}`
            );
            return res.status(200).json(result);
        }
